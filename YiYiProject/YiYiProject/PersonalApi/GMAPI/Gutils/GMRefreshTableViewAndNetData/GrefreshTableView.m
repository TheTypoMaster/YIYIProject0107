...
-(void)createHeaderView
{
    ...
    _refreshHeaderView = [[GrefreshTableHeaderView alloc]initWithFrame:CGRectMake(0.0f,0.0f -self.bounds.size.height, self.frame.size.width, self.bounds.size.height)];
...
-(void)showRefreshHeader:(BOOL)animated
{
    ...
    [_refreshHeaderView setState:GPullRefreshLoading];
    [_refreshHeaderView GRefreshScrollViewDidEndDragging:self];
}

#pragma mark - GRefreshTableDelegate
...
- (void)finishReloadigData
{
    ...
    if (_refreshHeaderView) {
        [self.refreshHeaderView GRefreshScrollViewDataSourceDidFinishedLoading:self];
...
{
    if (_refreshHeaderView) {
        [_refreshHeaderView GRefreshScrollViewDidScroll:scrollView];
    }
}
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    if (_refreshHeaderView)
    {
        [_refreshHeaderView GRefreshScrollViewDidEndDragging:scrollView];
    }
