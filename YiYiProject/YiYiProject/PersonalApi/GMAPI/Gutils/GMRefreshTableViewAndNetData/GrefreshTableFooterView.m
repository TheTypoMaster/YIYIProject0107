...
@implementation GrefreshTableFooterView
...
- (void)egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_state == GPullRefreshLoading) {
        ...
        if (_state == GPullRefreshPulling && scrollView.contentOffset.y + (scrollView.frame.size.height) < scrollView.contentSize.height + RefreshViewHight && scrollView.contentOffset.y > 0.0f && !_loading) {
            [self setState:GPullRefreshNormal];
        } else if (_state == GPullRefreshNormal && scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight  && !_loading) {
...
- (void)egoRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight && !_loading) {
        ...
        if (_delegate) {
            [_delegate GRefreshTableDidTriggerRefresh:GRefreshFooter];
        }
